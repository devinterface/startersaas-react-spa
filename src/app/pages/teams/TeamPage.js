...
export default function TeamPage({ user }) {
  ...
  const addTeamUser = async (userId) => {
    try {
      const result = await addTeamUserMutate.mutateAsync({
        teamId: teamId,
        userId: userId,
      });
    } catch (error) {
  ...
  const removeTeamUser = async (userId) => {
    try {
      const result = await removeTeamUserMutate.mutateAsync({
        teamId: teamId,
        userId: userId,
      });
    } catch (error) {
      ...
      {team && (
        <>
          ...
          <div className="d-flex flex-column flex-lg-row gap-4">
                ...
                <thead>
                  <tr className="d-none d-md-table-row">
                    <th
                      ...
                      className="col-1"
                    >
                      <div className="unchecked-user"></div>
                    </th>
                    <th
                      style={{ borderLeft: "0px", borderRight: "0px" }}
                ...
                {users && (
                  <tbody>
                    {users.data.map((user, id) => (
                      ...
                      >
                        {user.teams &&
                        user.teams.some((check) => check.id === teamId) ? (
                          ...
                          <td className="border-0">
                            <div className="d-flex justify-content-start align-items-center">
                              <div
                                ...
                                onClick={() => addTeamUser(user.id)}
                              />
                            </div>
                          </td>
                        )}
                        <div className="d-md-none fw-bold pb-0">#:</div>
                        <td className="text-start border-0 vertical-align">
                          {id + 1}
                        </td>
            ...
            <div
              className="dashboard-box mt-3"
              style={{ height: "max-content", maxWidth: "415px" }}
            >
              <Form
                onSubmit={(e) => {
                  e.preventDefault();
